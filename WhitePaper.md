...
<p>Il certificato di dominio Ephemeral funge anche da portafoglio Viat che memorizza i fondi per tutti i puzzle che distribuisce ai client. Una parte del Viat estratto viene inviata al portafoglio Viat che è la chiave pubblica archiviata all'interno dei certificati di dominio.</p>The domain's ephemeral certificate also acts as a Viat wallet that stores funds for any puzzles it distributes to clients. A portion of the mined Viat is sent to the Viat wallet that is the public key that is stored inside domain certificates.</p>
